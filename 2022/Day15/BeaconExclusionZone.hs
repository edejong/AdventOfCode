...
module Day15.BeaconExclusionZone where
...
main :: IO ()
main = do
    xs <- map parseLine . lines <$> readFile "2022/data/day15.txt"
    -- let row = 10
    let row = 2000000
        (lo, hi) = (0, 2*row)
    ...
    let rows = [lo..hi]
        ...
        [(y, [(_,x),_])] = filter (\(i,r) -> r /= [(lo,hi)]) $ zip [0..] . map (map (clampRange lo hi)) $ rss
        part2 = (x+1) * 4000000 + y
