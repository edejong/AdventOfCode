...
module Day11.MonkeyintheMiddle where
...
main = do
  ...
  where
    ...
    monkeyBusiness ms itemsMap worryFunc numRounds = product . take 2 . reverse . sort . map snd . M.toList . fst $ doRounds ms itemsMap worryFunc numRounds
...
doRound worryFunc monkeys (countMap, itemsMap) = foldl' (f worryFunc) (countMap, itemsMap) monkeys
  where
    f worryFunc (countMap, itemsMap) monkey@(Monkey id op test) = do
        let items = itemsMap ! id
            itemsMap1 = M.insert id [] itemsMap
