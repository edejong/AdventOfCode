...
import           Data.List           (foldl', inits, sort, tails)
...
main :: IO ()
main = do
    valves <- map parseValve . lines <$> readFile "2022/Day16/day16.txt"
...
removeValve vmap lbl =
    let valve = vmap ! lbl
        ...
        ts = map (\(Tunnel l lbl1, ts') -> (lbl1, map (\(Tunnel l2 lbl2) -> Tunnel (l+l2) lbl2) ts')) . choose . tunnels $ valve
