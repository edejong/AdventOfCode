...
main :: IO ()
main = do
    f <- readFile "2021/Day23/day23.txt"
...
isFinished (hallway, rooms) = all (=='.') hallway && and [all (==a) room | (a, room) <- zip ['A'..] rooms]
...
moves roomSize b@(hallway, rooms) = movesIntoHallway ++ movesIntoRooms
  where
    ...
    movesIntoHallway        = [move roomSize b r x | (r, _, range) <- unemptyRooms, x <- [fst range..snd range] \\ [2,4..8]]
...
move roomSize (hallway, rooms) r pos = (cost', (hallway', rooms'))
  where
